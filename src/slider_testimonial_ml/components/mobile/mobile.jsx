...
import { loadedImg2, dinamicRef } from '../../utils';
...
class Mobile extends React.Component {
    ...
    init = () => {
        ...
        for (let item of allSlide) {
            loadedImg2(item.current, this.updateSliderDimension, this.totalSlides, false);
        }
                        ...
                        {info.map((item, index) => {
                            return (
                                <div key={index} ref={this.refAllSlide[index]} className={style.StepContainerImages}>
                                    <img alt="step-one" data-src={item.image} className={style.ImageSlider}/>
                                    <Description key={item.id} item={item}></Description>
                                    <img alt="step-one" data-src={item.image_2} className={style.ImageSlider2}/>
                                </div>
                            )
                        })}
                    </div>
